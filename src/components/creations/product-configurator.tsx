...
export function ProductConfigurator({ catalog }: { catalog: CommerceCatalog }) {
  ...
  const [counts, setCounts] = useState<Record<string, number>>({
    [defaultCoating.id]: variants[0].pieceCount,
  });
  ...
  const hasValidAllocation =
    mode === "single" || hasCompleteCoatingAllocation(variant.pieceCount, counts);
  const error = !hasValidAllocation
    ? `Assign ${remainingPieces} more ${remainingPieces === 1 ? "piece" : "pieces"}.`
    : "";
  const coatingNames = useMemo(
    () => Object.fromEntries(coatings.map((coating) => [coating.id, coating.name])),
    [coatings],
  );
  const coatingPrices = useMemo(
  ...
  function submit() {
    ...
    addItem({
      ...
      coatingNames,
      coatingPrices,
      extraCoatingCharge: coatingCharge,
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_24rem] xl:grid-cols-[minmax(0,1fr)_27rem]">
          ...
          {coatings.map((coating) => {
              ...
              <>
                <div className="relative aspect-[4/3] overflow-hidden bg-surface-muted">
                  <Image
                    ...
                    alt={`${coating.name} coated chocolate-filled Litaw`}
                    fill
                    sizes="(min-width: 1280px) 18vw, (min-width: 640px) 35vw, 90vw"
                    className="object-cover transition-transform duration-300 group-hover:scale-[1.02]"
                  />
                  {selected ? (
                    <span className="absolute left-3 top-3 flex size-8 items-center justify-center rounded-full bg-brand text-surface shadow">
            ...
            return (
              <article
                key={coating.id}
                className={cn(
                  "group overflow-hidden rounded-card border bg-surface transition",
              ...
              >
                {mode === "single" ? (
                  <button
                    ...
                    onClick={() => chooseSingle(coating.id)}
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between border-t border-border px-4 py-3">
                      <span className="text-xs font-bold text-muted-foreground">Pieces</span>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          aria-label={`Remove one ${coating.name}`}
                          disabled={count === 0}
                          onClick={() => adjust(coating.id, -1)}
                          className="flex size-10 items-center justify-center rounded-full bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-35"
                        >
                        ...
                        <button
                          type="button"
                          aria-label={`Add one ${coating.name}`}
      ...
      >
        ...
        >
          <h2 id="build-box-heading" className="font-display text-3xl">
            Build your box
          </h2>
          ...
          <div className="mt-5 space-y-5">
            ...
            <div className="rounded-control bg-surface-control p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                {mode === "mixed" ? (
                  <span
                    className={cn(
                      "text-xs font-bold",
                      error ? "text-danger-foreground" : "text-success-foreground",
                    )}
                  >
              ...
              {mode === "single" ? (
                <p className="mt-2 font-bold">{selectedCoating.name}</p>
              ) : (
                <div className="mt-3 flex flex-wrap gap-2">
                  {coatings
                    .filter((coating) => (counts[coating.id] ?? 0) > 0)
                    .map((coating) => (
                      <span
                        key={coating.id}
                        className="inline-flex items-center gap-1 rounded-full bg-surface pl-3 pr-1 py-1 text-xs font-bold"
                      >
                        {coating.name} × {counts[coating.id]}
                        <button
                          type="button"
                          onClick={() => removeMixedCoating(coating.id)}
                          aria-label={`Remove ${coating.name} allocation`}
                          className="flex size-7 items-center justify-center rounded-full text-muted-foreground hover:bg-surface-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
                        >
            ...
            <div className="flex items-center justify-between border-t border-border pt-5">
              ...
              <strong className="font-display text-2xl">{formatPhp(unitTotal * quantity)}</strong>
            </div>
            <PrimaryButton
              type="button"
