  ...
  return (
    <div ref={rootRef} className={cn("relative min-w-0 space-y-2", className)}>
      ...
      <button ref={buttonRef} type="button" disabled={disabled} aria-labelledby={`${id}-label ${id}-value`} aria-haspopup="listbox" aria-expanded={open} aria-controls={`${id}-listbox`} data-invalid={Boolean(error) || undefined} onClick={() => { setActiveIndex(selectedIndex); setOpen((current) => !current); }} onKeyDown={(event) => {
        ...
        <span id={`${id}-value`} className="min-w-0 truncate">{selected?.label}</span><ChevronDown aria-hidden="true" className={cn("shrink-0 transition-transform", open && "rotate-180")} size={18} />
      </button>
      {open ? <ul id={`${id}-listbox`} role="listbox" aria-labelledby={`${id}-label`} className="absolute z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-control border border-border bg-surface p-1 shadow-xl">
