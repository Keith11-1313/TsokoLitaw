...
export function CartPageContent() {
  ...
  return <div className="grid items-start gap-8 lg:grid-cols-[1fr_22rem]"><section><h1 className="font-display text-4xl">Your cart</h1><p className="mt-2 text-muted-foreground">Choose the boxes you want to include, then continue to checkout.</p><label className="mt-6 flex min-h-11 items-center gap-3 text-sm font-bold"><input type="checkbox" checked={allSelected} onChange={(event) => setAllItemsSelected(event.target.checked)} className="size-5 accent-brand" />Select all ({selectedItemIds.length}/{items.length})</label><div className="mt-4 space-y-4">{items.map((item) => {
    ...
    return <article key={item.id} className={cn("rounded-card border bg-surface p-5 transition sm:p-6", selected ? "border-brand ring-2 ring-brand/10" : "border-border")}><div className="flex items-start gap-4"><input type="checkbox" checked={selected} onChange={(event) => setItemSelected(item.id, event.target.checked)} aria-label={`Select Box of ${item.pieceCount} for checkout`} className="mt-1 size-5 shrink-0 accent-brand" /><div className="min-w-0 flex-1"><div className="flex flex-col gap-5 sm:flex-row sm:justify-between"><div><p className="text-xs font-bold uppercase text-subtle-foreground">Box of {item.pieceCount}</p><h2 className="mt-1 font-display text-2xl">Chocolate Litaw box</h2><p className="mt-2 text-sm text-muted-foreground">{coatings}</p>{item.addonQuantity && item.addonName ? <p className="mt-1 text-sm text-muted-foreground">{item.addonName} × {item.addonQuantity} per box</p> : null}</div><strong className="font-display text-xl">{formatPhp(unitTotal * item.quantity)}</strong></div><div className="mt-5 flex flex-wrap items-end justify-between gap-4 border-t border-border pt-4"><QuantityInput label="Box quantity" value={item.quantity} onChange={(quantity) => updateQuantity(item.id, quantity)} min={1} max={MAX_CART_LINE_QUANTITY} className="w-32" /><button type="button" onClick={() => removeItem(item.id)} className="flex min-h-11 items-center gap-2 rounded-full px-3 text-sm font-bold text-danger-foreground hover:bg-danger-background"><Trash2 size={17} />Remove</button></div></div></div></article>;
