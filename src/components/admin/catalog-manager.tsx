...
function CoatingEditor({
  ...
  coating: AdminCatalogCoating | null;
  onClose: () => void;
}) {
  ...
  const { formRef, formProps, canSubmit, statusMessage, isDirty } = useFormGate({
    requireDirty: Boolean(coating),
    extraValid: !imageError && !imageChecking && Boolean(preview),
  });
  ...
  async function selectImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      setPreview(await readSquareImage(file));
    } catch (error) {
      setImageError(error instanceof Error ? error.message : "Choose a valid square image.");
    } finally {
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="coating-editor-title"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="coating-editor-title" className="mt-1 font-display text-3xl">
              {coating ? "Edit coating" : "Add coating"}
            </h2>
          </div>
          <button
            type="button"
            aria-label="Close coating editor"
            disabled={pending}
        ...
        >
          <input type="hidden" name="coatingId" value={coating?.id ?? ""} />
          <input type="hidden" name="existingImageUrl" value={coating?.imageUrl ?? ""} />
          <FormField
            id="coating-name"
            label="Name"
            required
          ...
          <NumberStepper
            ...
            max={10000}
            step={0.01}
            defaultValue={coating?.pricePerPiece ?? 5}
          />
          <FormField
          ...
          <label className="flex min-h-11 items-center gap-3 rounded-control bg-surface-control px-4 py-3 text-sm font-bold">
            <input
              type="checkbox"
              name="isActive"
              defaultChecked={coating?.isActive ?? true}
          ...
          <label className="flex min-h-11 items-center gap-3 rounded-control bg-surface-control px-4 py-3 text-sm font-bold">
            <input
              type="checkbox"
              name="isDefault"
              defaultChecked={coating?.isDefault ?? false}
              className="size-4 accent-brand"
            />
            Default coating
          </label>
  ...
  return (
    ...
    >
      <section
        ...
        aria-hidden={confirmDiscard || undefined}
        onPointerDown={(event) => event.stopPropagation()}
        className="my-auto w-full max-w-xl rounded-card border border-border bg-surface p-6 shadow-2xl sm:p-8"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-brand">
  ...
  return (
    <>
      <section className="grid gap-4 sm:grid-cols-3" aria-label="Catalog summary">
        <AdminStatCard
          compact
          label="Available box sizes"
          value={String(product.variants.filter((v) => v.isActive).length)}
        />
        <AdminStatCard
          compact
          label="Active coatings"
          value={String(coatings.filter((c) => c.isActive).length)}
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {coatings.map((coating) => (
            <article key={coating.id} className="rounded-card border border-border bg-surface p-5">
              <div
                className="aspect-square w-full rounded-control bg-surface-muted bg-cover bg-center"
                role="img"
                aria-label={`${coating.name} coating`}
                style={{ backgroundImage: `url(${coating.imageUrl ?? catalogImagePlaceholder})` }}
              />
              <div className="mt-4 flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span
                    className={`rounded-lg px-2.5 py-1 text-xs font-bold ${coating.isActive ? "bg-success-background text-success-foreground" : "bg-surface-muted text-muted-foreground"}`}
                  >
                    {coating.isActive ? "Active" : "Hidden"}
                  </span>
                  {coating.isDefault ? (
              ...
              <p className="mt-3 text-sm">
                Per piece: <strong>{formatPhp(coating.pricePerPiece)}</strong>
              </p>
              <SecondaryButton className="mt-5 w-full" onClick={() => setEditor(coating)}>
      ...
      <section className="mt-9" aria-labelledby="addons-heading">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          {addons.map((addon) => (
            <article key={addon.id} className="rounded-card border border-border bg-surface p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
      ...
      {addonEditor !== undefined ? (
        <AddonEditor
          key={addonEditor?.id ?? "new"}
          addon={addonEditor}
          onClose={() => setAddonEditor(undefined)}
        />
      ) : null}
    </>
  );
