...
export function AdminSidebar({ activePath, adminName = "Administrator", adminEmail = "Admin email to be configured", className }: { activePath?: string; adminName?: string; adminEmail?: string; className?: string }) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-50 bg-foreground/30 lg:hidden" onClick={() => setOpen(false)}><aside ref={drawerRef} aria-label="Admin menu" className="flex h-full w-[min(20rem,86vw)] flex-col overflow-y-auto bg-surface p-5 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><BrandLockup context="admin" /><button type="button" aria-label="Close admin menu" autoFocus onClick={() => setOpen(false)} className="flex size-11 items-center justify-center rounded-full bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><X aria-hidden="true" size={20} /></button></div>{navigation}<div className="mt-auto border-t border-border pt-5"><p className="text-sm font-bold">{adminName}</p><p className="truncate text-xs text-muted-foreground">{adminEmail}</p><Link href="/" className="mt-3 flex min-h-11 items-center gap-2 rounded-control text-sm font-bold text-brand"><Home size={17} />Back to main page</Link><LogoutButton className="mt-1 flex min-h-11 w-full items-center gap-2 rounded-control text-left text-sm text-muted-foreground hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus" /></div></aside></div> : null}
    <aside className={cn("sticky top-0 hidden h-screen shrink-0 flex-col border-r border-border bg-surface py-8 transition-[width,padding] lg:flex", collapsed ? "w-24 px-4" : "w-[17.5rem] px-8", className)}><div className={cn("flex items-center gap-3", collapsed ? "flex-col" : "justify-between")}><Link href="/admin" aria-label="Admin dashboard" className={cn(collapsed && "flex w-full justify-center")}><BrandLockup context="admin" showTitle={!collapsed} showSubtitle={!collapsed} /></Link><button type="button" title={collapsed ? "Expand sidebar" : "Collapse sidebar"} aria-label={collapsed ? "Expand admin sidebar" : "Collapse admin sidebar"} aria-expanded={!collapsed} onClick={() => setCollapsed((value) => !value)} className="flex size-10 shrink-0 items-center justify-center rounded-full bg-surface-muted text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{collapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</button></div>{navigation}<div className="mt-auto border-t border-border pt-6">{!collapsed ? <><p className="text-sm font-bold">{adminName}</p><p className="truncate text-xs text-muted-foreground">{adminEmail}</p></> : null}<Link title={collapsed ? "Back to main page" : undefined} href="/" className={cn("mt-3 flex min-h-11 items-center gap-2 rounded-control text-sm font-bold text-brand", collapsed && "justify-center")}><Home size={17} /><span className={cn(collapsed && "sr-only")}>Back to main page</span></Link><LogoutButton hideLabel={collapsed} className={cn("mt-1 flex min-h-11 w-full items-center gap-2 rounded-control text-left text-sm text-muted-foreground hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus", collapsed && "justify-center")} /></div></aside>
  </>;
}
