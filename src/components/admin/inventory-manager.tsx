...
import type { AdminInventoryDate, AdminInventoryRecord } from "@/lib/server-inventory";
...
function StockEditor({
  ...
  const { formRef, formProps, canSubmit, statusMessage, isDirty } = useFormGate({
    requireDirty: Boolean(record),
    extraValid: Boolean(record || dates.length),
  });
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <NumberStepper
            label="Total prepared pieces for this date"
            error={state.fieldErrors?.stockTotal}
            name="stockTotal"
            min={minimum}
            max={100000}
            step={1}
          ...
          {minimum > 0 ? (
            <span className="block text-xs font-normal text-muted-foreground">
              {minimum} pieces are already committed or removed. Use another pickup date to start a
              new independent stock limit.
            </span>
          ) : null}
        </div>
        ...
        <label className="space-y-2 text-sm font-bold sm:col-span-2">
          ...
          <input
            name="notes"
            maxLength={240}
            placeholder="Example: Morning batch added"
      ...
      <div className="mt-5 space-y-3">
        ...
        <div className={onCancel ? "grid gap-3 sm:grid-cols-2" : undefined}>
          {onCancel ? (
            <SecondaryButton disabled={pending} onClick={onCancel}>
              Cancel
            </SecondaryButton>
          ) : null}
...
function ConsumptionForm({ record }: { record: AdminInventoryRecord }) {
  ...
  const { formRef, formProps, canSubmit, statusMessage } = useFormGate({
    requireDirty: false,
    extraValid: record.stockAvailable > 0,
  });
  return (
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="publish-stock-modal-title"
        aria-hidden={confirmDiscard || undefined}
        onPointerDown={(event) => event.stopPropagation()}
        className="my-auto w-full max-w-3xl rounded-card bg-surface shadow-2xl"
      >
        <div className="flex items-start justify-between gap-4 px-5 pt-5 sm:px-7 sm:pt-7">
          ...
          <button
            ...
            className="flex size-11 items-center justify-center text-brand focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-50"
          >
            <X aria-hidden="true" />
          </button>
        </div>
      ...
      {selectedRecord ? (
        <section className="space-y-5" aria-labelledby="selected-stock-title">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <h2 id="selected-stock-title" className="font-display text-3xl">
                  {formatDate(selectedRecord.pickupDate)}
                </h2>
                {selectedDate ? (
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
              ...
              {unconfiguredDates.length > 0 ? (
                <PrimaryButton onClick={() => setShowPublishModal(true)}>
                  <Plus size={17} />
                  Publish stock for another date
                </PrimaryButton>
              ) : null}
          ...
          <div
            ...
            aria-label={`Inventory summary for ${formatDate(selectedRecord.pickupDate)}`}
          >
            <AdminStatCard compact label="Prepared" value={String(selectedRecord.stockTotal)} />
            <AdminStatCard
              compact
              label="Online committed"
              value={String(selectedRecord.stockReserved)}
            ...
            <AdminStatCard
              compact
              label="Available now"
              value={String(selectedRecord.stockAvailable)}
        ...
        <section
          ...
          aria-labelledby="empty-inventory-title"
        >
          <h2 id="empty-inventory-title" className="font-display text-2xl">
            No stock published yet
          </h2>
          ...
          {unconfiguredDates.length > 0 ? (
            <PrimaryButton className="mt-5" onClick={() => setShowPublishModal(true)}>
              <Plus size={17} />
              Publish stock
            </PrimaryButton>
          ) : (
            <Link href="/admin/pickup" className={`${secondaryButtonClassName} mt-5`}>
              Go to Pickup
            </Link>
          )}
        </section>
