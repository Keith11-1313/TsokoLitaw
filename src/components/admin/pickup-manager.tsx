...
import {
  ...
  savePickupScheduleAction,
  savePickupSettingsAction,
  setPickupDateOpenAction,
...
function ActionMessage({ state }: { state: PickupActionState }) {
  ...
  return (
    <p
      ...
      className={`rounded-control p-3 text-sm font-bold ${state.status === "error" ? "bg-danger-background text-danger-foreground" : "bg-success-background text-success-foreground"}`}
    >
      {state.message}
    </p>
  );
  ...
  function toggleLocation(key: string, locationId: string) {
    setWindows((current) =>
      current.map((window) => {
        ...
        return {
          ...window,
          locationIds: window.locationIds.includes(locationId)
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="pickup-editor-title"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="pickup-editor-title" className="mt-1 font-display text-3xl">
              {date ? "Edit pickup date" : "Add pickup date"}
            </h2>
          </div>
          <button
            type="button"
            aria-label="Close pickup editor"
            disabled={pending}
        ...
        <form ref={formRef} {...formProps} action={action} className="mt-6 space-y-6">
          ...
          <input
            ...
            value={JSON.stringify(
              windows.map(({ startTime, endTime, locationIds }) => ({
                startTime,
                endTime,
                locationIds,
              })),
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <FormField
              ...
              label="Pickup date"
              required
              inputProps={{ name: "pickupDate", type: "date", defaultValue: date?.pickupDate }}
            />
            <CustomSelect
              label="Availability mode"
              name="availabilityMode"
              required
              defaultValue={date?.availabilityMode ?? "MADE_TO_ORDER"}
              hint="Made to order accepts advance orders; Ready stock uses prepared pieces; Hybrid supports both."
            ...
            <FormField
              ...
              inputProps={{ name: "notes", maxLength: 500, defaultValue: date?.notes }}
            />
            <label className="flex min-h-11 items-center gap-3 rounded-control bg-surface-control px-4 py-3 text-sm font-bold sm:col-span-2">
              <input
                type="checkbox"
                name="isOpen"
                defaultChecked={date?.isOpen ?? true}
                className="size-4 accent-brand"
              />
              Publish this date at checkout
            </label>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <PrimaryButton type="submit" disabled={pending || !canSubmit}>
              {pending ? "Saving…" : date ? "Save schedule" : "Publish pickup date"}
            </PrimaryButton>
          </div>
        </form>
      </section>
    ...
    >
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <NumberStepper
          ...
          hint="Minimum full days needed before advance pickup."
          name="minimumLeadDays"
          required
          min={0}
          max={30}
          defaultValue={settings.minimumLeadDays}
        />
        <FormField
          id="cutoff-time"
        ...
        <FormField
          ...
          inputProps={{
            name: "operatingStart",
            type: "time",
            defaultValue: settings.operatingStart,
          }}
        />
        <FormField
          id="operating-end"
          label="Operating end"
          error={state.fieldErrors?.operatingEnd}
          required
          inputProps={{ name: "operatingEnd", type: "time", defaultValue: settings.operatingEnd }}
        />
      </div>
      <div className="mt-5 space-y-3">
        ...
        <FormStatusHint message={statusMessage} />
        <PrimaryButton type="submit" disabled={pending || !canSubmit}>
          {pending ? "Saving…" : "Save pickup rules"}
        </PrimaryButton>
      </div>
    </form>
  );
  ...
  return (
    ...
    >
      <section
        ...
        aria-hidden={confirmDiscard || undefined}
        onPointerDown={(event) => event.stopPropagation()}
        className="w-full max-w-xl rounded-card border border-border bg-surface p-6 shadow-2xl sm:p-8"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-brand">
            ...
            <h2 id="location-editor-title" className="mt-1 font-display text-3xl">
        ...
        <form ref={formRef} {...formProps} action={action} className="mt-6 space-y-5">
          ...
          <FormField
            ...
            label="Customer directions (optional)"
          ...
          <label className="flex min-h-11 items-center gap-3 rounded-control bg-surface-control px-4 py-3 text-sm font-bold">
            <input
              ...
              className="size-4 accent-brand"
            />
            Available for new pickup schedules
          </label>
          <ActionMessage state={state} />
          <FormStatusHint message={statusMessage} />
          <div className="grid gap-3 sm:grid-cols-2">
...
function PickupDateCard({
  date,
  locations,
  onEdit,
}: {
  date: AdminPickupDate;
  locations: AdminPickupLocation[];
  onEdit: () => void;
}) {
  ...
  return (
    <article className="rounded-card border border-border bg-surface p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="font-display text-2xl">{formatDate(date.pickupDate)}</h3>
            <span className="rounded-lg bg-surface-muted px-2.5 py-1 text-xs font-bold text-brand">
              {modeLabels[date.availabilityMode]}
            </span>
            <span
              className={`rounded-lg px-2.5 py-1 text-xs font-bold ${date.isOpen ? "bg-success-background text-success-foreground" : "bg-surface-muted text-muted-foreground"}`}
            >
              {date.isOpen ? "Published" : "Closed"}
            </span>
          </div>
          {date.notes ? <p className="mt-2 text-sm text-muted-foreground">{date.notes}</p> : null}
        </div>
        {date.isLocked ? (
          <span className="inline-flex items-center gap-2 text-xs font-bold text-muted-foreground">
            <LockKeyhole size={15} />
            Schedule locked by orders or inventory
          </span>
        ) : null}
      </div>
      <div className="mt-5 space-y-3">
        {date.windows.map((window) => (
          <div
            key={window.id}
            className="grid gap-2 rounded-control bg-surface-muted p-4 text-sm md:grid-cols-2"
          >
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <SecondaryButton disabled={date.isLocked || pending} onClick={onEdit}>
          <Pencil size={16} />
          Edit schedule
        </SecondaryButton>
        <SecondaryButton
          ...
          onClick={() =>
            startTransition(async () =>
              setMessage(
                await setPickupDateOpenAction({ pickupDateId: date.id, isOpen: !date.isOpen }),
              ),
            )
          }
        >
          {pending ? "Saving…" : date.isOpen ? "Close date" : "Publish date"}
        </SecondaryButton>
      </div>
      <div className="mt-3">
    ...
    <>
      ...
      {dates.length > 1 ? (
        <CustomSelect
          ...
          label="Manage pickup date"
          value={selectedDate?.id ?? ""}
          onChange={setSelectedDateId}
          options={dates.map((item) => ({
            value: item.id,
            label: `${formatDate(item.pickupDate)} · ${modeLabels[item.availabilityMode]}`,
          }))}
        />
      ) : null}
      <div className="mt-5">
        {selectedDate ? (
          <PickupDateCard
            ...
            date={selectedDate}
            locations={locations}
            onEdit={() => setEditor(selectedDate)}
          />
        ) : (
          <div className="rounded-card border border-border bg-surface p-10 text-center">
            <CalendarDays className="mx-auto text-brand" size={34} />
            <h3 className="mt-4 font-display text-2xl">No upcoming pickup dates</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Add a date before customers can choose a campus pickup.
            </p>
          </div>
        )}
      </div>
      <div className="mt-8">
        <PickupRules settings={settings} />
      </div>
      <section className="mt-8 rounded-card border border-border bg-surface p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {locations.map((location) => (
            <article key={location.id} className="rounded-control bg-surface-muted p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-bold">{location.name}</h3>
                    <span
                      className={`rounded-lg px-2 py-1 text-xs font-bold ${location.isActive ? "bg-success-background text-success-foreground" : "bg-surface text-muted-foreground"}`}
                    >
                ...
                <button
                  ...
                  onClick={() => setLocationEditor(location)}
                  className="flex size-11 shrink-0 items-center justify-center text-brand focus-visible:ring-2 focus-visible:ring-focus"
                >
      ...
      {locationEditor !== undefined ? (
        <LocationEditor
          key={locationEditor?.id ?? "new-location"}
          location={locationEditor}
          onClose={() => setLocationEditor(undefined)}
        />
      ) : null}
    </>
  );
