...
export function CheckoutContent({
  ...
  const selectedDate =
    availability.dates.find((date) => date.id === dateId) ?? availability.dates[0];
  const [windowId, setWindowId] = useState(selectedDate?.windows[0]?.id ?? "");
  const selectedWindow =
    selectedDate?.windows.find((window) => window.id === windowId) ?? selectedDate?.windows[0];
    ...
    return (
      <section className="mx-auto max-w-2xl rounded-card border border-border bg-surface p-7 text-center sm:p-10">
        ...
        {submission?.status === "error" ? (
          <p
            role="alert"
            className="mt-5 rounded-control bg-danger-background p-4 text-sm font-bold text-danger-foreground"
          >
            {submission.message}
          </p>
        ) : null}
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <Link
            href="/"
            className="inline-flex min-h-11 items-center justify-center rounded-full border border-brand px-6 text-sm font-bold text-brand"
          >
            Return home
          </Link>
        </div>
    ...
    return (
      <section className="rounded-card border border-border bg-surface p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Select at least one cart item before continuing.
        </p>
        <Link
          href="/cart"
          className="mt-6 inline-flex min-h-11 items-center rounded-full bg-brand px-6 font-bold text-surface"
        >
          Return to cart
        </Link>
      </section>
  ...
  const rewardDiscount =
    useReward && availableReward && eligibleRewardBoxes.length
    ...
    <div className="grid items-start gap-8 lg:grid-cols-[1.25fr_.75fr]">
      ...
      >
        <section className="rounded-card border border-border bg-surface p-6 sm:p-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            We’ll use your Google email as the main way to contact you about your order.
          </p>
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <FormField
              ...
              label="Full name"
              required
              error={submission?.fieldErrors?.customerName}
              inputProps={{
                value: customerName,
        ...
        {availableReward ? (
          <section
            ...
            aria-labelledby="checkout-reward-title"
          >
            <h2 id="checkout-reward-title" className="font-display text-2xl">
              Loyalty reward
            </h2>
          ...
          {hasPickupAvailability ? (
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <CustomSelect
                ...
                value={dateId}
                onChange={changeDate}
                options={availability.dates.map((date) => ({ value: date.id, label: date.label }))}
              />
              <CustomSelect
                label="Pickup time"
                value={selectedWindow.id}
                onChange={changeWindow}
                options={selectedDate.windows.map((window) => ({
                  value: window.id,
                  label: window.label,
                }))}
              />
              <CustomSelect
                ...
                options={selectedWindow.locations.map((location) => ({
                  value: location.id,
                  label: location.name,
                }))}
              />
              {remainingPieces !== null ? (
                <div
                  role={exceedsPreparedStock ? "alert" : "status"}
              ...
              <FormField
                ...
                error={submission?.fieldErrors?.customerNotes}
                className="sm:col-span-2"
                inputProps={{
                  value: customerNotes,
        ...
        {submission?.status === "error" ? (
          <p
            role="alert"
            className="rounded-control bg-danger-background p-4 text-sm font-bold text-danger-foreground"
          >
            {submission.message}
          </p>
        ) : null}
        {!customerDetailsValid ? (
          <p className="text-center text-xs font-bold text-danger-foreground">
            Complete the customer details before continuing.
          </p>
        ) : null}
        <PrimaryButton
          type="submit"
