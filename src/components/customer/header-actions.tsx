...
export function HeaderActions({ mobile = false }: { mobile?: boolean }) {
  ...
  const { isReady, isSignedIn, signOut } = useAuthPreview();
  ...
  return <div className={cn("flex items-center", mobile ? "w-full gap-3" : "gap-2")}>
    {isReady && isSignedIn ? <div ref={rootRef} className="relative">
      ...
      {open ? <div role="menu" className="absolute right-0 top-12 z-50 w-56 rounded-card border border-border bg-surface p-2 shadow-xl">
        ...
        <button role="menuitem" type="button" onClick={() => { setOpen(false); signOut(); router.replace("/login"); }} className="flex min-h-11 w-full items-center gap-3 rounded-control px-3 text-left text-sm hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><LogOut size={17} />Log out</button>
      </div> : null}
