...
export async function savePickupScheduleAction(
  ...
  try {
    windows = JSON.parse(String(formData.get("windows") ?? "[]")) as typeof windows;
  } catch {
    return { status: "error", message: "The pickup windows are invalid." };
  }

  const validTime = (value: string) => /^([01]\d|2[0-3]):[0-5]\d$/.test(value);
  if (
    (idValue && !isUuid(idValue)) ||
    ...
    windows.some(
      (window) =>
        !validTime(window.startTime) ||
        !validTime(window.endTime) ||
        window.endTime <= window.startTime ||
        !Array.isArray(window.locationIds) ||
        window.locationIds.length < 1 ||
        window.locationIds.some((id) => !isUuid(id)),
    )
  ) {
    return {
      status: "error",
      message: "Check the date, mode, time windows, and locations.",
      fieldErrors: {
        schedule:
          "Use a valid date, mode, and at least one correctly ordered window with a location.",
      },
    };
  }
  try {
    await guard(admin.id);
    await savePickupSchedule({
      adminId: admin.id,
      pickupDateId: idValue || null,
      pickupDate,
    ...
    return {
      status: "success",
      message: idValue ? "Pickup schedule updated." : "Pickup date published.",
    };
  } catch (error) {
    return failure(error, "Pickup schedule could not be saved.");
  }
}

export async function setPickupDateOpenAction(input: {
  pickupDateId: string;
  isOpen: boolean;
}): Promise<PickupActionState> {
  const admin = await requireAdmin("/admin/pickup");
  if (!isUuid(input.pickupDateId))
    return { status: "error", message: "That pickup date is invalid." };
  try {
    await guard(admin.id);
    ...
    return {
      status: "success",
      message: input.isOpen ? "Pickup date published." : "Pickup date closed.",
    };
  } catch (error) {
    return failure(error, "Pickup date visibility could not be changed.");
  }
}
...
export async function savePickupSettingsAction(
  ...
  try {
    ...
    return failure(error, "Pickup rules could not be saved.");
  }
}
...
export async function savePickupLocationAction(
  ...
  if (
    ...
    return {
      status: "error",
      message: "Use a name with 2 to 100 characters and keep the description under 300 characters.",
      fieldErrors: {
        ...(name.length < 2 || name.length > 100
          ? { name: "Use a name between 2 and 100 characters." }
          : {}),
  ...
  try {
    ...
    return {
      status: "success",
      message: idValue ? "Pickup location updated." : "Pickup location added.",
    };
  } catch (error) {
    return failure(error, "Pickup location could not be saved.");
  }
}
