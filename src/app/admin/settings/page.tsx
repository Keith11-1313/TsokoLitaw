...
export default function AdminSettingsPage() {
  return (
    <AdminPageLayout activePath="/admin/settings" title="Settings" description="Configure mock operational values; TsokoLitaw brand identity is fixed." actions={<PrimaryButton type="button" disabled title="Saving requires backend persistence"><Save size={17} />Save Changes</PrimaryButton>}>
      <form className="grid gap-6 xl:grid-cols-2" aria-label="Admin settings">
        <section className="rounded-card border border-border bg-surface p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-5">
            <FormField id="settings-support-email" label="Support Email" inputProps={{ type: "email", defaultValue: "hello@tsokolitaw.ph" }} />
            <FormField id="settings-location" label="Primary Location" inputProps={{ defaultValue: "UCC North Congress Campus, Caloocan" }} />
          </div>
        </section>
        <section className="rounded-card border border-border bg-surface p-6 sm:p-8">
          <h2 className="font-display text-2xl">Order configuration</h2>
          <div className="mt-6 space-y-5">
            <UncontrolledCustomSelect label="Daily cut-off" options={[{ value: "4", label: "4:00 PM" }, { value: "5", label: "5:00 PM" }]} />
            <FormField id="settings-lead-days" label="Pickup Lead Days" inputProps={{ type: "number", defaultValue: 2, min: 1 }} />
