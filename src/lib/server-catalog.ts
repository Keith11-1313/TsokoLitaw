...
export async function getAdminCatalog() {
  ...
  if (productResult.error || !productResult.data || coatingsResult.error || addonsResult.error) {
    throw new Error("The Admin catalog could not be loaded.", {
      cause: productResult.error ?? coatingsResult.error ?? addonsResult.error,
    });
  }
  ...
  const coatings: AdminCatalogCoating[] = (coatingsResult.data ?? []).map((coating, index) => ({
    ...
    isActive: coating.is_active,
    sortOrder: coating.sort_order,
  }));
...
export async function saveCatalogCoating(input: {
  ...
  const { error } = await createAdminSupabaseClient().rpc("upsert_catalog_coating", {
    target_admin_id: input.adminId,
    target_coating_id: input.coatingId,
    name_value: input.name,
    description_value: input.description,
    image_url_value: input.imageUrl,
    price_per_piece_value: input.pricePerPiece,
    active_value: input.isActive,
...
export async function saveCatalogAddon(input: {
  adminId: string;
  addonId: string | null;
  name: string;
  price: number;
...
export async function uploadCatalogImage(input: { adminId: string; file: File }) {
  ...
  if (error) {
    ...
    if (
      ...
      (detail.includes("not found") || detail.includes("does not exist"))
    ) {
      throw new Error("Catalog media storage is not configured in this environment.", {
        cause: error,
      });
    }
    if (
      ...
      throw new Error("Catalog media storage is not authorized in this environment.", {
        cause: error,
      });
    }
...
export async function removeCatalogImage(path: string) {
  ...
  if (error)
    throw new Error("The newly uploaded catalog image could not be cleaned up.", { cause: error });
}
