...
import { parsePayMongoPaidEvent, verifyPayMongoWebhookSignature } from "./paymongo-webhook";
...
describe("PayMongo webhook verification", () => {
  it("verifies the timestamped test signature against the untouched body", () => {
    ...
    expect(verifyPayMongoWebhookSignature(body, signatureFor(body), SECRET, "test", NOW)).toBe(
      true,
    );
    expect(
      verifyPayMongoWebhookSignature(`${body} `, signatureFor(body), SECRET, "test", NOW),
    ).toBe(false);
  });
  ...
  it("rejects stale and live-only signatures", () => {
    ...
    expect(
      verifyPayMongoWebhookSignature(body, signatureFor(body), SECRET, "test", NOW + 301_000),
    ).toBe(false);
    expect(
      verifyPayMongoWebhookSignature(body, `t=${TIMESTAMP},te=,li=${digest}`, SECRET, "test", NOW),
    ).toBe(false);
    expect(
      verifyPayMongoWebhookSignature(body, signatureFor(body, "live"), SECRET, "live", NOW),
    ).toBe(true);
    expect(verifyPayMongoWebhookSignature(body, signatureFor(body), SECRET, "live", NOW)).toBe(
      false,
    );
  });
  ...
  it("ignores unrelated events and rejects live or malformed paid events", () => {
    expect(
      parsePayMongoPaidEvent(
        { event_type: "send.webhook", data: { type: "payment.failed" } },
        "test",
      ),
    ).toBeNull();
  ...
  it("does not treat a refund event as a paid checkout", () => {
    expect(
      parsePayMongoPaidEvent(
        { data: { type: "event", attributes: { type: "payment.refunded", livemode: false } } },
        "test",
      ),
    ).toBeNull();
  });
