...
| `prepare_paymongo_checkout`, `attach_paymongo_checkout`, `replace_paymongo_checkout`, `process_paymongo_paid_event` | `server-payment.ts`, PayMongo route    | Initial                              | One payment, provider-expired compare-and-replace, exact provider/order/amount match and paid deduplication   |
...
| `get_admin_customer_summaries`, `count_admin_customers`                                                             | `server-customers.ts`                  | Initial                              | Authorized, bounded server-side customer search and pagination                                                |
