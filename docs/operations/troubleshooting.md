...
| Head hydration mismatch                      | Compare the server/client diff, search the repository for the differing text, then reload with browser extensions isolated | Assume deterministic font CSS or suppress hydration across the body    |

For a mismatch at the root font `<style>`, a client-side value such as `body[unresolved]` that does
not exist in the repository is evidence of browser or extension injection, not changing application
copy. Confirm with a fresh reload and browser console before changing code. The root layout tolerates
head-only injection; a mismatch under `<body>` still requires investigation and must not be hidden.
