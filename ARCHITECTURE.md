...
The Supabase foundation defines the PostgreSQL schema, RLS policies, tests, controlled seeds, and browser/server/privileged client helpers. Google OAuth and authenticated profile access are connected. Active products, variants, coatings, add-ons, and published pickup options load from Supabase. Phase 9 validates and reprices checkout server-side, reserves inventory, creates immutable pending-order snapshots, records Terms acceptance, prevents duplicate submissions, and expires overdue unpaid reservations. Phase 10 creates idempotent PayMongo checkout sessions restricted to QR Ph, redirects customers to Hosted Checkout, verifies signed paid webhooks, and treats the browser return as informational only. Provider-bound expiry is coordinated server-side so stock is released only after PayMongo closes the checkout session. Phase 11 connects audited Admin fulfillment, reviews, Journal, Catalog, Inventory, Pickup, Customers, and the cross-feature dashboard. Phase 12 adds atomic loyalty plus durable event-driven email. Active order triggers enqueue confirmation, ready, and unpaid cancellation. Historical refund triggers and signed-event reconciliation remain intact for older transactions, but the current website does not create refunds. Trusted server code sends through Resend immediately when possible and a secret-protected scheduled endpoint retries bounded local send failures without changing commerce state. Resend delivery callbacks enter through a raw-body signature-verified endpoint; a database function deduplicates provider event IDs and applies only the newest provider event to each delivery.
...
Online cancellation is allowed only while the order and payment are pending. Unpaid cancellations release reserved stock; paid orders expose no website cancellation or refund control and are settled directly with TsokoLitaw in person. Prepared and no-show orders are non-refundable, subject to non-waivable rights. Historical refund lifecycle data remains separately tracked for older transactions.

The order-detail server action asks the database for a locked unpaid-cancellation preparation. For a provider-bound order it expires the exact PayMongo checkout before the database changes order/payment state or releases inventory. The database rejects paid orders even if a client tampers with the request. No current server action creates a refund, calls the provider refund API, or collects a destination. Historical refund rows and signed webhook handling remain for reconciliation only.
...
- Keep checkout, unpaid cancellation, webhook, expiry, inventory, and historical refund-reconciliation transitions in bounded atomic database functions with the smallest practical lock scope. Never weaken locking to gain speed.

### Concurrency, backpressure, and failure handling

- Preserve idempotency keys for checkout creation, unpaid cancellation, webhooks, scheduled jobs, and historical refund reconciliation. A retry must return or advance the same operation, not create a second one.
...
- Keep payment-provider work outside long-held database locks. Record a durable intent, call the provider, then apply an idempotent verified transition. Historical refund events remain idempotent but no current application path calls the refund API.
