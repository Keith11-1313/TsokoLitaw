...
Customer Profile shows completed-order progress toward the next configured reward plus the number of available free boxes. Admin Customers presents a compact customer directory with the same persisted progress, available rewards, and successfully redeemed reward count. Checkout exposes reward redemption only when an available reward and a selected 4-piece box both exist, then shows the discount as a separate summary line before the final total.
